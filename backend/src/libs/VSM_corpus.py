...
def load_data(path='src/data/Corpus_Convert', query=False):
    path_dataset = os.path.join(path,'corpus_3162_doc.json')
    with open(path_dataset, encoding="UTF-8") as f:
        dataset = json.load(f)

    if not query:
        path_dataset = os.path.join(path,'corpus_325_result.json')
        with open(path_dataset, encoding="UTF-8") as f:
            results = json.load(f)

        path_query = os.path.join(path, 'corpus_325_query.json')
...
def get_tf(tokens, docToken):
    ...
    for x in docToken:
        ...
        for word,count in doc_dict[str(x)].items():
            tf_doc_dict[int(x)][word]= math.log(1+ count)
...
def get_idf(tokens, docToken):
    ...
    for word in tokens:
        if word_count[word]>0:
            ...
            if count>3162: #Có 3162 tài liệu
                count=3162
        # idf_Dict[word]=math.log(3162/count)### Câu 5: term + tf idf lưu xuống file Term.txt

        # idf_Dict[word]=(1/count) #Công thức idf  
        # idf_Dict[word]=math.log(3162/count) #Công thức idf
        idf_Dict[word]=math.log((3162/count)+1) #Công thức idf    
    return idf_Dict
...
def get_tf_idf(docToken, doc_dict, tf_doc_dict, idf_Dict):
    ...
    for x in docToken:
        ...
        for word in doc_dict[str(x)]:
            tf_idf[x][word]=tf_doc_dict[int(x)][word]*idf_Dict[word]
    return tf_idf
...
def handle_process(path='src/data/Corpus_Convert', save_path='', save=False):
    ...
    if save:
        write_file(os.path.join(save_path, 'VSM_Corpus_token.txt'), tokens)
        write_file(os.path.join(save_path, 'VSM_Corpus_idf_Dict.txt'), idf_Dict)
        write_file(os.path.join(save_path, 'VSM_Corpus_tf_idf.txt'), tf_idf)